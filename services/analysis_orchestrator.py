...
class AnalysisOrchestrator:
    ...
    def scan_directory(self, 
        ...
        Delega el trabajo a DirectoryScanner.
        
        Args:
            directory: Directorio a escanear
            progress_callback: Función opcional de progreso
            create_metadata_cache: Si crear caché de metadatos
            precalculate_hashes: Si pre-calcular hashes
        
        Returns:
            DirectoryScanResult con archivos clasificados y caché opcional
        """
        # Delegar a DirectoryScanner
        scanner = DirectoryScanner()
        return scanner.scan(
            directory=directory,
            progress_callback=progress_callback,
            create_metadata_cache=create_metadata_cache,
    ...
    def analyze_live_photos(self,
        ...
        # El nuevo servicio usa analyze(metadata_cache, ...)
    ...
    def analyze_heic_duplicates(self,
        ...
        return heic_service.analyze(metadata_cache, progress_callback=progress_callback, directory=directory)
    ...
    def analyze_zero_byte_files(self,
        ...
        Args:
            ...
            progress_callback: Función opcional de progreso
            **kwargs: Argumentos adicionales (metadata_cache)
            
        Returns:
            ZeroByteAnalysisResult con archivos vacíos encontrados
        """
        self.logger.info("Buscando archivos de 0 bytes")
        # ZeroBytesService usa metadata_cache, pero lo pasamos como kwarg
        # porque su firma espera directory como argumento principal?
        # Revisemos ZeroByteService: analyze(self, directory, progress_callback, metadata_cache=None)
        # Asi que podemos pasarlo directo.
        
        return zero_byte_service.analyze(
            directory=directory, 
            progress_callback=progress_callback, 
    ...
    def run_full_analysis(self,
        ...
        if zero_byte_service:
            ...
            result.zero_byte, result.phase_timings['zero_byte'] = self._execute_phase(
                ...
                phase_callable=lambda: self.analyze_zero_byte_files(directory, zero_byte_service, progress_callback, metadata_cache=metadata_cache),
                phase_callback=phase_callback,
                partial_callback=partial_callback
            )
