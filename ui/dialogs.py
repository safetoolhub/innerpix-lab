...
class NormalizationPreviewDialog(QDialog):
    """Diálogo de preview para normalización"""
    ...
    def init_ui(self):
        self.setWindowTitle("Preview de Normalización")
        self.setModal(True)
        self.resize(900, 600)
        layout = QVBoxLayout(self)

        # Tabla de cambios propuestos
        if self.analysis_results.get('normalization_plan'):
            ...
            plan = self.analysis_results['normalization_plan'][:50]
            ...
            if len(self.analysis_results['normalization_plan']) > 50:
                more_label = QLabel(f"... y {len(self.analysis_results['normalization_plan']) - 50} más")
        ...
        if self.analysis_results.get('need_normalization', 0) > 0:
            buttons.button(QDialogButtonBox.Ok).setText(
                f"Proceder ({self.analysis_results['need_normalization']})"
            )
        else:
    ...
    def accept(self):
        self.accepted_plan = {
            'plan': self.analysis_results['normalization_plan'],
