...
from PyQt5.QtWidgets import (
    ...
    QTabWidget, QWidget, QTextEdit, QGroupBox, QProgressBar, QLineEdit
)
...
def show_results_html(window, html: str, show_generic_status: bool = False):
    try:
        if show_generic_status:
            try:
                window.app_logger.info('Operación completada — revisa el log para detalles')
            except Exception:
                try:
...
def reset_analysis_ui(window, reinsert_analyze=True):
    ...
    try:
        try:
            window.app_logger.info("Directorio cambiado: análisis anterior limpiado")
        except Exception:
            try:
    ...
    window.app_logger.info("UI reiniciada tras cambio de directorio")
