# This file is part of Innerpix Lab, licensed under GPLv3 with
...
def package_linux(output_dir: Path, full_version: str) -> list[Path]:
    ...
    desktop_id = "org.safetoolhub.innerpixlab"
    ...
    metainfo_src = ROOT / "build" / "org.safetoolhub.innerpixlab.metainfo.xml"
    metainfo_dest = staging / "usr" / "share" / "metainfo"
    metainfo_dest.mkdir(parents=True)
    if metainfo_src.exists():
        shutil.copy2(metainfo_src, metainfo_dest / "org.safetoolhub.innerpixlab.metainfo.xml")
    ...
    if dpkg_deb:
        ...
        if license_src.exists():
            copyright_text = textwrap.dedent(f"""\
                ...
                Upstream-Name: Innerpix Lab
                Upstream-Contact: SafeToolHub <contact@safetoolhub.org>
                Source: https://github.com/safetoolhub/innerpix-lab
...
def package_appimage(output_dir: Path, full_version: str) -> list[Path]:
    ...
    # Application files → AppDir/opt/innerpix-lab/
    ...
    desktop_src = ROOT / "build" / "org.safetoolhub.innerpixlab.desktop"
    desktop_dst = appdir / f"org.safetoolhub.innerpixlab.desktop"
    if desktop_src.exists():
        ...
        desktop_dst.write_text(textwrap.dedent(f"""\
            ...
            Name=Innerpix Lab
            Exec=innerpix-lab
            Icon=org.safetoolhub.innerpixlab
            Categories=Graphics;Photography;
            Comment=Privacy-first photo and video management
            Terminal=false
        """))
    ...
    icon_src = ROOT / "assets" / "icon.png"
    if icon_src.exists():
        shutil.copy2(icon_src, appdir / "org.safetoolhub.innerpixlab.png")
    ...
    metainfo_src = ROOT / "build" / "org.safetoolhub.innerpixlab.metainfo.xml"
    if metainfo_src.exists():
        shutil.copy2(metainfo_src, metainfo_dir / "org.safetoolhub.innerpixlab.metainfo.xml")
        shutil.copy2(metainfo_src, metainfo_dir / "org.safetoolhub.innerpixlab.appdata.xml")

    # Run appimagetool
    appimage_name = f"InnerPixLab-{full_version}-linux-x86_64.AppImage"
