...
@pytest.fixture
def dialog(qapp, sample_analysis):
    """Create a FileOrganizationDialog instance for testing"""
    dialog = FileOrganizationDialog(sample_analysis)
    yield dialog
    dialog.close()
    dialog.deleteLater()
...
class TestStrategySelection:
    ...
    def test_date_strategy_selection(self, dialog):
...
class TestLogicIntegration:
    ...
    def test_date_granularity_change_triggers_analysis(self, dialog, monkeypatch):
    ...
    def test_checkbox_change_triggers_analysis(self, dialog, monkeypatch):
        ...
        dialog.chk_date_source.setChecked(True)
        dialog._on_option_changed(None)
        
        assert len(analysis_called) > 0
        assert analysis_called[0]['group_by_source'] is True
    
    def test_type_strategy_reads_secondary_grouping(self, dialog, monkeypatch):
...
class TestComboBoxArrow:
    ...
    def test_date_granularity_combo_is_visible(self, dialog):
        """Test that date granularity combo is visible when date strategy is selected"""
        dialog._on_strategy_clicked('date')
        
        # The combo should be in the visible page
        assert dialog.settings_stack.currentWidget() == dialog.date_settings_page
        assert dialog.date_granularity_combo.isVisible()
...
class TestDefaultSelection:
    ...
    def test_default_does_not_preselect_if_no_initial_analysis(self, qapp, temp_dir, create_test_image):
        """Test behavior when no initial analysis is provided"""
        # Create minimal analysis
        create_test_image(temp_dir / "test.jpg", size=(100, 100))
        organizer = FileOrganizer()
        analysis = organizer.analyze(temp_dir, OrganizationType.BY_MONTH)
...
class TestAnalysisWorkflow:
    ...
    def test_worker_creation(self, dialog, monkeypatch):
        """Test that worker is created correctly"""
        workers_created = []
        
        original_worker = dialog.worker.__class__ if hasattr(dialog, 'worker') and dialog.worker else None
    ...
    def test_analysis_updates_tree(self, dialog, monkeypatch):
...
class TestEdgeCases:
    ...
    def test_rapid_strategy_switching(self, dialog):
    ...
    def test_rapid_combo_changes(self, dialog):
    ...
    def test_checkbox_toggle_multiple_times(self, dialog):
